...
const SchoolPage = () => {
  ...
  style={{
    marginRight: '70rem', // تحريك العنصر لليمين
    marginButtom: '7rem', // تحريك العنصر للأسفل
    position: 'relative', // إذا كنت بحاجة لتحديد موضعه بشكل نسبي
  }}
>
  
<span className="flex items-start justify-between  ">
  <div className="flex -mt-6 mr-5"> {/* رفع لفوق أكتر وتحريك لليسار */}
  {Array.from({ length: 5 }, (_, index) => (
      <svg
        key={index}
  ...
  <ul className="space-y-6">
    {comments.length > 0 ? (
      comments.map((comment, index) => (
        <li
          key={index}
          className=" max-w-md mx-auto bg-white shadow-md  font-cairo rounded-xl p-6  text-lg text-navy1 border border-gray"
        >
   ...
   {isRatingPopupOpen && (
        <div className="fixed inset-0 bg-gray  bg-opacity-50 flex items-center justify-center z-10">
          <div className="bg-white p-6 rounded-lg w-1/3 shadow-lg " >
            ...
            <button
              onClick={handleRatingSubmit}
              className="px-4 py-2 bg-bluee text-white rounded-lg top:10px"
              style={{ margintop: "100px" }}
            >
              إرسال التقييم
            ...
            <button
              onClick={() => setIsRatingPopupOpen(false)}
              style={{ marginRight: 400  }}
              className="text-gray bg-redd mt-4 px-4 py-2 rounded-lg  "
            >
              إغلاق
            </button>
          </div>
